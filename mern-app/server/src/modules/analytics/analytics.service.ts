...
export async function getOverview(query: AnalyticsQuery) {
  ...
  const [
    ...
    Grievance.countDocuments({ ...dateFilter, status: { $nin: ['CLOSED', 'REJECTED'] } }),
...
export async function getAuditLogs(query: AuditLogsQuery) {
  const { page, limit, entityType, entityId, actorId, action, dateFrom, dateTo } = query;
...
function buildDateFilter(dateFrom?: string, dateTo?: string): Record<string, unknown> {
...
async function getAvgResolutionDays(dateFilter: Record<string, unknown>): Promise<number | null> {
