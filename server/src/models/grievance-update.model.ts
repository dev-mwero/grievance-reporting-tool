...
const grievanceUpdateSchema = new Schema<IGrievanceUpdate>(
  {
    ...
    authorId: {
      type: Schema.Types.ObjectId,
      ref: 'User',
      // null for anonymous/system updates
    },
    authorName: {
      type: String,
...
// Index for querying updates by grievance
grievanceUpdateSchema.index({ grievanceId: 1, createdAt: -1 });
// Index for filtering by type
grievanceUpdateSchema.index({ grievanceId: 1, type: 1, createdAt: -1 });

export const GrievanceUpdate = mongoose.model<IGrievanceUpdate>(
  'GrievanceUpdate',
  grievanceUpdateSchema
);
